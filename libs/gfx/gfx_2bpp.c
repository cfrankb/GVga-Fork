...
{
    uint tmp;
    if (x0 >= gvga->width || x1 >= gvga->width || y0 >= gvga->height || y1 >= gvga->height)
        return NULL;
    int dy = y1 - y0;
    if (dy == 0)
        return hline(gvga, x0, y0, x1, pen);
    int dx = x1 - x0;
    if (dx == 0)
        return vline(gvga, x0, y0, y1, pen);
    if (x1 < x0)
    {
        _swap(x0, x1);
        _swap(y0, y1);
...
static void *drawText(GVga *gvga, uint x, uint y, char *text, uint pen, uint bgn)
{
    pen &= 0x03;
    uint rowBytes = gvga->rowBytes;
...
static Gfx _gfx_2bpp = {
    ...
    .rect = rect,
    .rectFill = rectFill,
    .drawText = drawText}; // TODO: FIX THIS LATER
